...
export default function Preview({ searchParams }) {
    ...
    useEffect (() => {
        ...
        const getfunc = async() => {
            const { data, names, dataln } = await getpreview(i);
            setData(data);
            setNames(names);
            setDataln(dataln);
        }
                ...
                <Container>
                    <div className="relative w-full h-full">
                            ...
                            <div className="mx-6 mb-6 sm:mx-36 md:mx-48 text-white text-xl sm:text-xl">
                                    <span className={style['underlines']}>All Volumes</span>
                            </div>
                            ...
                            <div className="text-white text-lg text-center sm:text-left sm:text-xl sm:ml-48 md:ml-60">
                                <div className={style['link']}>
                                    <ul className="pb-10">
                                        ...
                                        <li key={datas.id}>
                                            <Link href={`/Lightnovel/content?i=${i}&v=${names}&id=${datas.ids}`} className="text-white hover:underline focus:underline">{data.name},  {datas.volume}</Link>
                                        </li>
                                        )}
                                    </ul>
                                </div>
