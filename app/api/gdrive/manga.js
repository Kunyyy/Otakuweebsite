...
const uploads = async(data) => {
    ...
    if (imageslinks && deskripsi && jp && title) {
        let dbpush = await prisma.manga.create({
            data: {
                name: title,
                alias: alias,
...
const contentUploads = async(content) => {
    ...
    const trimmedChapter = split[1].trimStart();
    ...
    let foundDuplicate = false;
        ...
        if (trimmedChapter && !foundDuplicate) {
            
            const checkdb = await prisma.mangaContent.findMany({
                where: {
                    title: trimmedName,
                }
            })

            const existingRecord = await prisma.mangaContent.findFirst({
                where: {
                    title: trimmedName,
            ...
            if (!existingRecord) {
                ...
                let dbpush = await prisma.mangaContent.create({
                    data: {
                        title: trimmedName,
