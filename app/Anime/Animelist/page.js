...
export default function Completed() {
    ...
    useEffect(() => {
        ...
        const getAni = async () => {
            const { completedanimes, animes } = await getAnime();
            setCompletedanime(completedanimes);
            setLoading(false);
        };
        getAni();
    ...
    const totalPages = Math.ceil(completedanime.length / itemsPerPage);

    // Calculate the index of the last item on the current page
    const lastIndex = currentPage * itemsPerPage;

    // Get the current page of items
    const currentItems = completedanime.slice(
            ...
            <div className="relative w-full h-full">
                <Container>
                    <div className="text-bold pt-5 px-5 flex justify-between">
                        <span className="text-2xl text-white">Completed Anime</span>
                    </div>
